...
\begin{abstract}
ICU monitors sample vital signs every few seconds. The standard interoperability format for those readings is HL7 FHIR Observation. General-purpose time-series databases such as InfluxDB and TimescaleDB ingest the data but drop the clinical semantics at the storage layer, which forces reconstruction at query time. \textbf{EmberDB} is a Rust time-series engine for FHIR Observation data. Records carry a composite metric key of patient identifier, LOINC code, and unit. Data is partitioned into 1-hour chunks and persisted through a write-ahead log. Five clinical pattern-detection algorithms run inside the engine: CUSUM changepoint detection, PELT segmentation, seasonal decomposition, Mahalanobis anomaly scoring, and moving-window analysis. On a synthetic MIMIC-IV workload of 500 patients, 48-hour ICU stays, and 1.7M chart\-events, EmberDB sustains 3.2M records/sec in memory mode with sub-microsecond point queries. Against a SQLite baseline on patient-scoped workloads it runs 7-9$\times$ faster on ingestion and 8{,}890$\times$ faster on point lookup.
\end{abstract}
...
On the full-patient-stay query in the MIMIC workload, SQLite outperforms EmberDB (617~$\mu$s vs.\ 945~$\mu$s) because EmberDB issues separate queries per vital metric while SQLite returns all vitals in a single index scan.
